...
{
    public class RabbitMQDistributedEventSubscriber : IDistributedEventSubscriber
    {
        ...
        private string _exchangeTopic = "DistributedEventBus.Exchange.Topic";
        private string _queuePrefix = "DistributedEventBus.Queue.";

        private bool _disposed;

        public RabbitMQDistributedEventSubscriber(IRabbitMQSetting rabbitMQSetting)
        {
        ...
        public void Subscribe(IEnumerable<string> topics, Action<string, string> handler)
        {
            ...
            foreach (var topic in topics)
            {
                ...
                try
                {
                    ...
                    {
                        handler(ea.RoutingKey, Encoding.UTF8.GetString(ea.Body.ToArray()));
